...
def verify_hardware(hardware, cpu_data, gpu_data):
    ...
    if hardware["cpu"] not in cpu_data:
        print("\nYour CPU is not in our database.")
        benchmark = int(input(f"Enter the benchmark score for your CPU ({hardware['cpu']}): "))
        cpu_data[hardware["cpu"]] = {"benchmark": benchmark}
        save_json(cpu_data, "cpu_data.json")
        print(f"CPU '{hardware['cpu']}' has been added to the database.")

    # Verify GPU
    if hardware["gpu"] not in gpu_data:
        print("\nYour GPU is not in our database.")
        benchmark = int(input(f"Enter the benchmark score for your GPU ({hardware['gpu']}): "))
        gpu_data[hardware["gpu"]] = {"benchmark": benchmark}
        save_json(gpu_data, "gpu_data.json")
        print(f"GPU '{hardware['gpu']}' has been added to the database.")
...
def main():
    while True:
        print("\nWelcome to the Game Benchmark Checker!")
        ...
        # Ask the user to search again or exit
        next_action = input("\nWould you like to search again or exit? (search/exit): ").strip().lower()
        if next_action == "exit":
